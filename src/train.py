...
def main_worker(gpu_idx, configs):
    ...
    input = torch.rand((1999,100,4,), device='cpu')
    input1 = torch.rand((1999,4),device='cpu')
    input2 = torch.rand(1999, device='cpu')

    #if tb_writer is not None:
     #   tb_writer.add_graph(model, (input, input1, input2))
    ...
    # Create dataloader
    train_dataloader, train_sampler = create_train_dataloader(configs, voxel_generator)
    if logger is not None:

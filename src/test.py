...
from utils.visualization_utils import merge_rgb_to_bev, show_rgb_image_with_boxes, show3dlidar
...
def parse_test_configs():
    ...
    ####################################################################
    configs.root_dir = '../'
    configs.dataset_dir = '/media/wx/Software/BaiduNetdiskDownload/dataset/videodata'
...
def get_lidar(lidar_dir):
    lidar_file = os.path.join(lidar_dir)
...
if __name__ == '__main__':
    ...
    test_dataloader = create_test_dataloader(configs, voxel_generator)
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(test_dataloader):
            metadatas = batch_data
            ...
            pointcloud = get_lidar(img_path.replace(".png", ".bin").replace("image_2", "velodyne"))

...
from models import resnet, fpn_resnet, pose_dla_dcn, pose_dla_dcn_level2


def create_model(configs, voxel_generator):
    ...
    if 'fpn_resnet' in configs.arch:
        ...
        model = fpn_resnet.get_pose_net(voxel_generator= voxel_generator, num_layers=num_layers, heads=configs.heads, head_conv=configs.head_conv,
        ...
        model = resnet.get_pose_net(num_layers=num_layers, heads=configs.heads, head_conv=configs.head_conv,
                                    imagenet_pretrained=configs.imagenet_pretrained)
    elif 'dla' in configs.arch:
        print('using DLA34 architecture')
        model = pose_dla_dcn_level2.get_pose_net(voxel_generator= voxel_generator, num_layers=num_layers, heads=configs.heads)
    else:
